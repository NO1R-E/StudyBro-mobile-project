...
const Timetable = ({ navigation }) => {
  ...
  const [editingSubjectOriginalCode, setEditingSubjectOriginalCode] = useState(null);
  ...
  const handleEditSubjectClick = (itemCode) => {
    const subjectClasses = table.filter((c) => c.code === itemCode && c.table === selectedTable);
    if (subjectClasses.length > 0) {
      const baseClass = subjectClasses[0];
      setSubject({ code: baseClass.code, name: baseClass.name, sec: baseClass.sec });
  ...
  const handleDeleteEntireSubject = () => {
    Alert.alert("ยืนยันการลบวิชา", `คุณต้องการลบวิชา ${subject.code} ทั้งหมดออกจากตารางเรียนใช่หรือไม่?`, [
      { text: "ยกเลิก", style: "cancel" },
      {
        text: "ลบวิชา",
        style: "destructive",
        onPress: () => {
          const updatedTable = table.filter(
            (item) => !(item.code === editingSubjectOriginalCode && item.table === selectedTable)
          );
  ...
  return (
    <View style={styles.container}>
                  ...
                  dailyClasses.map((item) => (
                    <View key={item.id} style={styles.classCard}>
                      <View style={{ flexDirection: "row", gap: 15 }}>
                        <Text style={[styles.timeLabel, { color: theme?.text, width: 90 }]}>
                          {item.start} - {item.end}
                        </Text>
                        <View style={{ flex: 1 }}>
                          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                            <Text style={[styles.classlabel, { color: theme?.detail, fontFamily: "Inter_700Bold", flex: 1 }]} numberOfLines={2}>
                              {item.code} sec {item.sec} {item.type ? `(${item.type})` : ''}
                            </Text>
                            <TouchableOpacity onPress={() => handleEditSubjectClick(item.code)} style={{ padding: 6, flexDirection: 'row', alignItems: 'center', backgroundColor: '#F1F2F6', borderRadius: 8, flexShrink: 0 }}>
                              <Feather name="edit" size={14} color="#00B894" style={{ marginRight: 4 }} />
                              <Text style={{ fontFamily: "Inter_700Bold", color: "#00B894", fontSize: 12 }}>แก้ไข</Text>
                            </TouchableOpacity>
                          </View>
      ...
      <Modal visible={modalSubjectVisible} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { maxHeight: '90%', paddingBottom: 15 }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ flexShrink: 1 }}>
              ...
              <TextInput placeholder="หมู่ (SEC)" style={styles.input} value={subject.sec} onChangeText={(t) => setSubject({ ...subject, sec: t })} />

              <Text style={[styles.label, { fontSize: 16, marginTop: 10, marginBottom: 10, color: '#C7005C' }]}>
              ...
              {sessions.map((session, index) => (
                <View key={session.id} style={styles.sessionCard}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10, alignItems: 'center' }}>
                    <Text style={{ fontFamily: "Inter_700Bold", color: "#C7005C" }}>คาบที่ {index + 1}</Text>
                    {sessions.length > 1 && (
                      <TouchableOpacity onPress={() => setSessions(sessions.filter(s => s.id !== session.id))} style={{ padding: 5 }}>
                  ...
                  <View style={{ flexDirection: "row", marginTop: 10 }}>
                    <View style={{ flex: 1, marginRight: 5 }}>
                      ...
                      <TouchableOpacity style={styles.pickerButton} onPress={() => setActivePicker({ index, field: 'start' })}>
                        <Text style={styles.pickerText}>{formatTime(session.startTime)}</Text>
                        <Ionicons name="time-outline" size={20} color="gray" />
                      </TouchableOpacity>
                    </View>
                    <View style={{ flex: 1, marginLeft: 5 }}>
                      <Text style={styles.label}>เวลาสิ้นสุด</Text>
                      <TouchableOpacity style={styles.pickerButton} onPress={() => setActivePicker({ index, field: 'end' })}>
                        <Text style={styles.pickerText}>{formatTime(session.endTime)}</Text>
                        <Ionicons name="time-outline" size={20} color="gray" />
                      </TouchableOpacity>
                    </View>
                  </View>
              ...
              <TouchableOpacity 
                ...
                onPress={() => setSessions([...sessions, { 
                  id: Date.now().toString(), day: "Monday", type: "Lab", room: "", 
                  startTime: getDefaultStartTime(), endTime: getDefaultEndTime() 
                }])}
              >
                <Text style={styles.addSessionBtnText}>+ เพิ่มคาบเรียนใหม่ (Lec / Lab)</Text>
              </TouchableOpacity>
            </ScrollView>

            {activePicker && (
              <DateTimePicker
                value={sessions[activePicker.index][`${activePicker.field}Time`]}
                mode="time"
                display="default"
                onChange={(e, date) => {
                  ...
                  if (date) {
                    const newS = [...sessions];
                    newS[currentPicker.index][`${currentPicker.field}Time`] = date;
                    setSessions(newS);
                  }
                }}
              />
            ...
            <View style={[styles.modalActions, { marginTop: 15 }]}>
              <TouchableOpacity style={styles.saveBtn} onPress={handleAddSubject}>
                <Text style={styles.saveBtnText}>{isEditingSubject ? "บันทึกการเปลี่ยนแปลง" : "บันทึกวิชา"}</Text>
              </TouchableOpacity>

              {/* ปุ่มลบวิชาทั้งหมด จะแสดงเฉพาะตอนแก้ไข */}
              {isEditingSubject && (
                <TouchableOpacity style={[styles.saveBtn, { backgroundColor: "#FF7675" }]} onPress={handleDeleteEntireSubject}>
...
const styles = StyleSheet.create({
  ...
  sessionCard: { backgroundColor: "#FDF2F8", padding: 15, borderRadius: 15, marginBottom: 15, borderWidth: 1, borderColor: "#FFDAE0" },
  addSessionBtn: { backgroundColor: "#FFF", borderWidth: 1.5, borderColor: "#C7005C", borderStyle: "dashed", padding: 12, borderRadius: 12, alignItems: "center", marginBottom: 10 },
  addSessionBtnText: { color: "#C7005C", fontFamily: "Inter_700Bold", fontSize: 14 }
});
